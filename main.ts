...
serve(async (req: Request): Promise<Response> => {
  ...
  // Keep local fallback for syllabus only.
  const syllabusFile = `syllabi/${body.course}syllabus.md`;
  const syllabus =
    body.syllabus ??
    (await Deno.readTextFile(syllabusFile).catch(() => "Error loading syllabus."));

  // FIX: do NOT assume assessments/<course>.md exists.
  // The browser sends assessment content; if it doesn't, just pass empty text.
  ...
  const messages = [
    ...
    ...(assessment
      ? [{
          role: "system",
          content: `Here are the assessment details:\n${assessment}`,
        }]
      : []),
    {
      role: "user",
